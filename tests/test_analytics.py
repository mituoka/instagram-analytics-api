...
class TestAnalyticsEndpoints:
    ...
    def test_get_influencer_keywords_success(self, mock_get_keywords, api_test_client, mock_keywords):
    ...
    def test_get_engagement_keywords_exception_handling(self, mock_get_db, mock_analyze, api_test_client):
        ...
        # APIリクエスト
        response = api_test_client.get("/api/v1/analytics/keywords/engagement?type=likes&limit=10")
    ...
    @patch("app.routers.analytics.text_analysis_service.analyze_keywords_by_engagement")
    @patch("app.routers.analytics.get_db")
    def test_get_engagement_keywords_value_error(self, mock_get_db, mock_analyze, api_test_client):
        ...
        # APIリクエスト
        response = api_test_client.get("/api/v1/analytics/keywords/engagement?type=invalid_type&limit=10")
    ...
    @patch("app.routers.analytics.text_analysis_service.analyze_keywords_by_engagement")
    @patch("app.routers.analytics.get_db")
    def test_get_engagement_keywords_count_exception(self, mock_get_db, mock_analyze, api_test_client):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["total_analyzed_posts"] == 100  # デフォルト値
