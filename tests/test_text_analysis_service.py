...
class TestTextAnalysisService:
    ...
    def test_get_influencer_keywords(self, mock_extract_nouns, mock_db_session):
        ...
        assert f"Influencer with ID 999 not found" in str(excinfo.value.detail)

    @patch("app.services.text_analysis_service.extract_nouns")
    @patch("app.services.text_analysis_service.datetime")
    def test_get_trending_keywords(self, mock_datetime, mock_extract_nouns, mock_db_session):
        """トレンドキーワード取得のテスト"""
        # キャッシュをクリア
        cache.clear()
        ...
        mock_posts = [
            ...
            MagicMock(text="インスタグラムの新機能", post_id=2, post_date=now - timedelta(days=10)),
    ...
    def test_analyze_keywords_by_engagement(self, mock_extract_nouns, mock_db_session):
        ...
        assert any(item["word"] == "エンゲージメント" and item["count"] == 1 for item in like_result)
        ...
        assert any(item["word"] == "いいね" and item["count"] == 1 for item in comment_result)
